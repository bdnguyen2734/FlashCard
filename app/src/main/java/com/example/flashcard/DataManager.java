    ...
    public class DataManager {
        ...
        public static void resetCategoryProgress(WordListActivity wordListActivity, String currentCategoryName) {
        }

        // ==========================================
        // 1. DATABASE HELPER (SQLite)
        // ==========================================
        private static class LocalDbHelper extends SQLiteOpenHelper {
            private static final String DB_NAME = "flashcard_internal.db";
            // Tăng version lên 4 để thêm cột Image
            ...
            static final String COL_WORD_IMAGE = "image_url"; // Cột mới để lưu link ảnh
            ...
            public void onCreate(SQLiteDatabase db) {
                ...
                db.execSQL("CREATE TABLE " + TABLE_WORD + " (" +
                        ...
                        COL_WORD_CAT_ID + " INTEGER, " +
                        COL_WORD_ENGLISH + " TEXT, " +
                        COL_WORD_VN + " TEXT, " +
                        COL_WORD_DEF + " TEXT, " +
                        COL_WORD_LEARNED + " INTEGER DEFAULT 0, " +
                        COL_WORD_IMAGE + " TEXT, " +
                        "FOREIGN KEY(" + COL_WORD_CAT_ID + ") REFERENCES " + TABLE_CAT + "(" + COL_CAT_ID + "))");
            ...
            public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
                // Nâng cấp DB: Thêm cột nếu chưa có
                if (oldVersion < 3) {
                    try { db.execSQL("ALTER TABLE " + TABLE_USER + " ADD COLUMN " + COL_USER_NAME + " TEXT"); } catch (Exception e) {}
                }
                if (oldVersion < 4) {
                    try { db.execSQL("ALTER TABLE " + TABLE_WORD + " ADD COLUMN " + COL_WORD_IMAGE + " TEXT"); } catch (Exception e) {}
                }
                // Fallback
        ...
        public static String getUserDisplayName(Context context) {
            ...
            try {
                Cursor cursor = db.query(LocalDbHelper.TABLE_USER, new String[]{LocalDbHelper.COL_USER_NAME},
                        LocalDbHelper.COL_USER_EMAIL + " = ?", new String[]{email}, null, null, null);
        ...
        public static void updateDisplayName(Context context, String newName) {
            ...
            db.update(LocalDbHelper.TABLE_USER, values,
                    LocalDbHelper.COL_USER_EMAIL + " = ?", new String[]{email});
            db.close();
        }
        ...
        public static int[] getLearningStats(Context context) {
            ...
            try {
                ...
                // Số từ đã học
                Cursor cLearned = db.rawQuery("SELECT COUNT(*) FROM " + LocalDbHelper.TABLE_WORD +
                        " WHERE " + LocalDbHelper.COL_WORD_LEARNED + " = 1", null);
        ...
        public static boolean registerUser(Context context, String email, String password) {
            ...
            Cursor cursor = db.query(LocalDbHelper.TABLE_USER, null,
                    LocalDbHelper.COL_USER_EMAIL + " = ?", new String[]{email}, null, null, null);
        ...
        public static boolean checkLogin(Context context, String email, String password) {
            ...
            SQLiteDatabase db = dbHelper.getReadableDatabase();
            Cursor cursor = db.query(LocalDbHelper.TABLE_USER, null,
                    LocalDbHelper.COL_USER_EMAIL + " = ? AND " + LocalDbHelper.COL_USER_PASS + " = ?",
        ...
        public static boolean loginOrRegisterWithGoogle(Context context, String googleEmail) {
            ...
            SQLiteDatabase db = dbHelper.getWritableDatabase();
            Cursor cursor = db.query(LocalDbHelper.TABLE_USER, null,
                    LocalDbHelper.COL_USER_EMAIL + " = ?", new String[]{googleEmail}, null, null, null);
            boolean exists = cursor.getCount() > 0;
            cursor.close();
        ...
        // Cập nhật hàm này để lưu ảnh vào DB
            ...
            try {
                ...
                for (Category cat : allCategories) {
                    for (Word w : cat.getWords()) {
                        ...
                        values.put(LocalDbHelper.COL_WORD_IMAGE, w.getImageUrl()); // Lưu link ảnh

                        // Cập nhật dựa trên từ tiếng Anh
                        db.update(LocalDbHelper.TABLE_WORD, values,
                                LocalDbHelper.COL_WORD_ENGLISH + " = ?",
                                new String[]{w.getEnglishWord()});
                    }
                }
        ...
        public static List<Category> loadData(Context context) {
            ...
            if (count == 0) {
                importFromAssetsAndSaveToDb(context, dbHelper);
            } else {
                loadFromDb(db);
            }
            db.close();
        ...
        public static void updateWordProgress(Context context, Word word) {
            LocalDbHelper dbHelper = new LocalDbHelper(context);
            SQLiteDatabase db = dbHelper.getWritableDatabase();
            ContentValues values = new ContentValues();
            values.put(LocalDbHelper.COL_WORD_LEARNED, word.isLearned() ? 1 : 0);
            db.update(LocalDbHelper.TABLE_WORD, values, LocalDbHelper.COL_WORD_ENGLISH + " = ?", new String[]{word.getEnglishWord()});
            db.close();
        }
                ...
                do {
                    ...
                    if (cw.moveToFirst()) {
                        do {
                            ...
                            int idxImg = cw.getColumnIndex(LocalDbHelper.COL_WORD_IMAGE); // Cột ảnh
                            ...
                            if (idxImg != -1) w.setImageUrl(cw.getString(idxImg)); // Load ảnh
            ...
            try {
                ...
                for (Map.Entry<String, List<Word>> entry : dataMap.entrySet()) {
                    ...
                    for (Word word : rawWords) {
                        ...
                        // Mặc định ảnh null khi mới import
        ...
        private static String englishWordToVn(String eng, String cat) {
            return eng + " (TV)"; // Logic giả
        }

        private static Map<String, String> initializeCategoryNames() {
            Map<String, String> map = new HashMap<>();
            map.put("animals", "Động vật"); map.put("fruits", "Hoa quả");
            map.put("vegetables", "Các loại rau"); map.put("food_drinks", "Thức ăn và đồ uống");
            map.put("school", "Trường học"); map.put("jobs", "Nghề nghiệp");
            map.put("clothing", "Quần áo"); map.put("home_furniture", "Nhà cửa & Nội thất");
            map.put("body_parts", "Bộ phận cơ thể"); map.put("colors", "Màu sắc");
            map.put("sports", "Thể thao"); map.put("weather", "Thời tiết");
            map.put("transportation", "Phương tiện giao thông"); map.put("technology", "Công nghệ");
            map.put("nature", "Thiên nhiên"); map.put("emotions", "Cảm xúc");
            map.put("hobbies", "Sở thích"); map.put("travel", "Du lịch");
